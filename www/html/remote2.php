      ...
      <div class="row">
        ...
        <div class="col-md-4 col-sm-6 col-xs-12">
          <div class="box box-danger">
            ...
            <div class="box-body">
                <div style="height:250px; vertical-align:middle;text-align: center" class="text-center"><div style="height:250px;width:250px;display:inline-block" id="staticDrive"></div></div>
            </div>
            <div class="box-body">
                <div style="height:250px; vertical-align:middle;text-align: center" class="text-center"><div style="height:250px;width:250px;display:inline-block" id="staticTurn"></div></div>
            </div>
...
<script>
    ...
    var managerDrive = nipplejs.create({
        zone: document.getElementById('staticDrive'),
        mode: 'static',
    ...
    var managerTurn = nipplejs.create({
        zone: document.getElementById('staticTurn'),
        mode: 'static',
        position: {left: '50%', top: '50%'},
        color: 'green',
        size: 128,
    ...
    var lt = 0;
    //Left Only
    managerDrive.on('move dir start end', function (evt, data) {
      var r = 0;
      var l = 0;
      if (data.distance) {
      ...
      if (movecnt > 1 && (new Date().getTime() / 1000 - lt > 0.3 || evt.type == "end") && evt.type != "start") {
        if (evt.type != "end") {
            $.post(api+"setmotorspeed", {right: 0, left: Math.round(l), stop: $("#stopcheck").is(':checked') ? "float" : "brake"}, function(result) {
              //console.log(r + " " + l);
            });
        } else {
    ...
    //Right Only
    managerTurn.on('move dir start end', function (evt, data) {
      var r = 0;
      var l = 0;
      if (data.distance) {
        var d = data.angle.degree;
